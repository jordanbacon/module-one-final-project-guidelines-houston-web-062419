...
def trait
    name = $prompt.ask("So what kind of value")
end

def waiting
    puts "In the meantime, how about having a look at the history of the houses and  see who is in each house?"
    waiting_list = $prompt.select("Which would you like to see?", %w(Houses Roster))

    if waiting_list == "Houses"
        house_data = $prompt.select("Which House info would you like to see?", %w(House_Name Founder))
        
        if house_data == "House_Name"
            House.all.each do |house|
                puts house.house_name
            end
        else
            House.all.each do |founders|
                puts founders.founder
            end
        end
    else
